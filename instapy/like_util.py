...
def _after_like_hook(after_like_hook, browser, result):
    if after_like_hook:
        element = (browser
...
def like_image(browser, username, blacklist, logger, logfolder, total_liked_img, after_like_hook):
    """Likes the browser opened image"""
    # check action availability
    if quota_supervisor("likes") == "jump":
        _after_like_hook(after_like_hook, browser, False)
        return False, "jumped"
    ...
    if len(like_elem) == 1:
        ...
        if len(liked_elem) == 1:
            ...
            _after_like_hook(after_like_hook, browser, True)
            return True, "success"
